...
class CarsDetection(object):
    ...
    def get_heatmap(self, img, shape, bbox_list, boxes_confident, show=False):
        ...
        for box, confident in zip(bbox_list, boxes_confident):
            # Add += 1*confident for all pixels inside each bbox
            # Assumingclusters.app. each "box" takes the form ((x1, y1), (x2, y2))
            heatmap[box[0][1]:box[1][1], box[0][0]:box[1][0]] += np.exp(confident)
            find_cluster = False
            for c, cluster in enumerate(clusters):
                if self._overlap(cluster, box):
                    x1 = min(box[0][0], cluster[0][0])
                    x2 = max(box[1][0], cluster[1][0])
                    y1 = min(box[0][1], cluster[0][1])
                    y2 = max(box[1][1], cluster[1][1])
                    clusters[c] = [[x1, y1], [x2, y2]]
                    find_cluster = True
            if not find_cluster:
                clusters.append(box)
        ...
        if show:
            plot_image(heatmap, cmap="gray", title="Without threshold")
        heatmap_without_t = np.copy(heatmap)

        threshold = max(heatmap[heatmap != 0].mean(), 10.0)
        #print("threshold", threshold)
        heatmap[heatmap <= threshold] = 0

        heatmap_with_t = np.copy(heatmap)
        if show:
            plot_image(heatmap, cmap="gray", title="With threshold")
        ...
        for cluster in clusters:
            is_import = False
            if heatmap[cluster[0][1]:cluster[1][1], cluster[0][0]:cluster[1][0]].mean() > 0.:
                cv2.rectangle(img, (cluster[0][0], cluster[0][1]), (cluster[1][0], cluster[1][1]), (0,240,0),6)
                final_clusters.append(cluster)
                self.last_clusters_center.append([(cluster[0][0] + cluster[1][0])//2, (cluster[0][1] + cluster[1][1])//2])
        """
        ...
        if show:
            plot_image(img, title="Clusters")


        return final_clusters, img, heatmap_without_t, heatmap_with_t
    ...
    def _create_new_box(self, image, xb, yb):
        """
            Method used to create a new box for this image
            **input:
                *image: Image class
                *xb: x step in the grid
                *yb: y step in the grid
        ...
        for s, scale in enumerate(self.SCALES):
            ...
            for xb in range(images[s].nxsteps):
                ...
                for yb in range(images[s].nysteps):
                    if x_progress > 0.15 and x_progress < 0.85:
                        ...
                        if (len(dists) == 0 or np.min(dists) > 150.) and not self.first:
                            continue
            ...
            #if show:
            #plot_image(draw_img)
        clusters, img, heatmap_without_t, heatmap_with_t = self.get_heatmap(
            np.copy(img), draw_img.shape, boxes, boxes_confident, show=show)

        heatmap_without_t = cv2.resize(heatmap_without_t, (300, 200))
        heatmap_with_t = cv2.resize(heatmap_with_t, (300, 200))

        img[0:200,0:300,:] = heatmap_without_t
        img[0:200,310:610,:] = heatmap_with_t

        #print("Number of possible cluster to place", len(clusters))

        for cluster in clusters:
            #print("cluster", cluster)
            car_found = False
            for car in self.cars:
                #print("car.box", car.box)
                if self._overlap(car.box, cluster):
                    #print("Overlap found for this cluster")
                    car_found = True
                    car.add_tmp_cluster(cluster)
                    break
            if not car_found:
                #print("No car found for this cluster. Create a new car")
    ...
    def draw_boxes(self, img):
        ...
        for car in self.cars:
            ...
            if car.active and np.mean(car.activity) <= 0.9:
                #print("Decrase confident of car")
                car.confident -= 0.6
            if car.active:
                cv2.putText(img, "%.2f - %.2f" % (car.confident, np.mean(car.activity)), (car.box[0][0] - 10, car.box[0][1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2, cv2.LINE_AA)
                cv2.rectangle(img, (car.box[0][0], car.box[0][1]), (car.box[1][0], car.box[1][1]), (100, 0, 0), 6)
            else:
                pass
                cv2.putText(img, "%.2f - %.2f" % (car.confident, np.mean(car.activity)), (car.box[0][0] - 10, car.box[0][1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2, cv2.LINE_AA)
                cv2.rectangle(img, (car.box[0][0], car.box[0][1]), (car.box[1][0], car.box[1][1]), (0, 0, 100), 6)
            #print("Active: %s, confident: %s, mean activity : %s" % (car.active, car.confident, np.mean(car.activity)))
    ...
    def find_cars(self, img, show=False, rescale=True):
        ...
        if global_i < 0:
            ...
            # Create list of images to look on
